<?php
class Tm_Builder_Product_Attribute extends Tm_Builder_Module {

	function init() {
		$this->name = esc_html__( 'Product Attribute', 'power-builder-integrator' );
		$this->icon = 'f07a';
		$this->slug = 'tm_pb_product_attribute';
	...
	function get_fields() {
		...
		$fields = array(
			...
			'attribute' => array(
				'label'       => esc_html__( 'Attribute', 'power-builder-integrator' ),
				'type'        => 'text',
				'description' => '',
			),
			'filter' => array(
				'label'       => esc_html__( 'Filter', 'power-builder-integrator' ),
				'type'        => 'text',
				'description' => '',
			),
			'admin_label' => array(
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$callback = power_builder_integrator()->get_shortcode_cb( 'woocommerce', 'product_attribute' );
		...
		$content = call_user_func(
			...
			array(
				...
				'attribute' => $this->_var( 'attribute' ),
				'filter'    => $this->_var( 'filter' ),
			)
		);
...
new Tm_Builder_Product_Attribute;
