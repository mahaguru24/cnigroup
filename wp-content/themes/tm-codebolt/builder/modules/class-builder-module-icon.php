 ...
 */
class Tm_Codebolt_Builder_Module_Icon extends Tm_Builder_Module {

	public $function_name;

	public function init() {
		$this->name = esc_html__( 'Icon', 'tm-codebolt' );
		$this->icon = 'f1d8';
	...
	public function get_fields() {
		...
		$fields = array(
			...
			'use_circle' => array(
				'label'           => esc_html__( 'Circle Icon', 'tm-codebolt' ),
				'type'            => 'yes_no_button',
				...
				'description'     => esc_html__( 'Here you can choose whether icon set above should display within a circle.', 'tm-codebolt' ),
			),
			'circle_color' => array(
				'label'       => esc_html__( 'Circle Color', 'tm-codebolt' ),
				'type'        => 'color-alpha',
				'description' => esc_html__( 'Here you can define a custom color for the icon circle.', 'tm-codebolt' ),
			),
			'use_circle_border' => array(
				'label'           => esc_html__( 'Show Circle Border', 'tm-codebolt' ),
				'type'            => 'yes_no_button',
				...
				'description'     => esc_html__( 'Here you can choose whether if the icon circle border should display.', 'tm-codebolt' ),
			),
			'circle_border_color' => array(
				'label'       => esc_html__( 'Circle Border Color', 'tm-codebolt' ),
				'type'        => 'color-alpha',
				'description' => esc_html__( 'Here you can define a custom color for the icon circle border.', 'tm-codebolt' ),
			),
			'animation' => array(
				'label'           => esc_html__( 'Icon Animation', 'tm-codebolt' ),
				'type'            => 'select',
				'option_category' => 'configuration',
				'options'         => array(
					'top'    => esc_html__( 'Top To Bottom', 'tm-codebolt' ),
					'left'   => esc_html__( 'Left To Right', 'tm-codebolt' ),
					'right'  => esc_html__( 'Right To Left', 'tm-codebolt' ),
					'bottom' => esc_html__( 'Bottom To Top', 'tm-codebolt' ),
					'off'    => esc_html__( 'No Animation', 'tm-codebolt' ),
				),
				'description'     => esc_html__( 'This controls the direction of the lazy-loading animation.', 'tm-codebolt' ),
			),
			'icon_orientation' => array(
				'label'           => esc_html__( 'Icon Orientation', 'tm-codebolt' ),
				'type'            => 'select',
				'option_category' => 'layout',
				'options'         => array(
				...
				'description'     => esc_html__( 'This will control how your icon is aligned.', 'tm-codebolt' ),
			),
			...
			'circle_size' => array(
				'label'           => esc_html__( 'Circle Size', 'tm-codebolt' ),
				'type'            => 'range',
				'option_category' => 'layout',
				'tab_slug'        => 'advanced',
				'default'         => '100',
				...
				'description'     => esc_html__( 'Here you can define a custom diameter for the icon circle.', 'tm-codebolt' ),
				'mobile_options'  => true,
				'depends_default' => true,
			),
			'circle_border_width' => array(
				'label'           => esc_html__( 'Circle Border Width', 'tm-codebolt' ),
				'type'            => 'range',
				'option_category' => 'layout',
				'tab_slug'        => 'advanced',
				'default'         => '2',
				...
				'description'     => esc_html__( 'Here you can define a custom width for the icon circle border.', 'tm-codebolt' ),
				'depends_default' => true,
			),
	...
	public function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		if ( '' !== $this->_var( 'font_icon' ) ) {
			...
			$content = sprintf(
				'<span class="tm_codeboltpb-icon tm_codeboltwaypoint%2$s%3$s%4$s" style="%5$s" data-icon="%1$s"></span>',
				$icon,
				esc_attr( ' tm_pb_animation_' . $animation ),
				( 'on' === $this->_var( 'use_circle' ) ? ' tm_codeboltpb-icon-circle' : '' ),
				( 'on' === $this->_var( 'use_circle' ) && 'on' === $this->_var( 'use_circle_border' ) ? ' tm_codeboltpb-icon-circle-border' : '' ),
				$icon_style
			);
		}
	...
	public function set_circle_size() {
		...
		if ( ! empty( $circle_size_d ) || ! empty( $circle_size_l ) || ! empty( $circle_size_t ) || ! empty( $circle_size_ph ) ) {
			...
			tm_pb_generate_responsive_css( $sizes, '%%order_class%% .tm_codeboltpb-icon', 'width', $this->function_name );
			tm_pb_generate_responsive_css( $sizes, '%%order_class%% .tm_codeboltpb-icon', 'height', $this->function_name );
			tm_pb_generate_responsive_css( $sizes, '%%order_class%% .tm_codeboltpb-icon', 'line-height', $this->function_name );
			tm_pb_generate_responsive_css( $radius, '%%order_class%% .tm_codeboltpb-icon', 'border-radius', $this->function_name );
		}
	}
}

new tm_codebolt_Builder_Module_Icon;
