 ...
 * @return array
 */
function tm_codebolt_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! tm_codebolt_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'tm_codebolt_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'tm_codebolt_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'tm-codebolt' ) . $network['name'],
 ...
 * @return string
 */
function tm_codebolt_get_single_share_buttons() {
	ob_start();
	tm_codebolt_share_buttons( 'single' );
