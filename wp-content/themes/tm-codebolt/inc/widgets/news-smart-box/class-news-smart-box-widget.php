...
/**
 * Class tm_codebolt_News_Smart_Box_Widget
 */
class tm_codebolt_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'tm_codebolt_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'tm-codebolt' );
		$this->widget_description = esc_html__( 'News Smart Box', 'tm-codebolt' );
		$this->utility            = tm_codebolt_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'tm-codebolt' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'tm-codebolt' ),
						'img_src' => TM_CODEBOLT_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'tm-codebolt' ),
						'img_src' => TM_CODEBOLT_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'tm-codebolt' ),
						'img_src' => TM_CODEBOLT_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'tm-codebolt' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'tm-codebolt' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'tm-codebolt' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'tm-codebolt' ),
				'placeholder'      => esc_html__( 'Select tag', 'tm-codebolt' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'tm-codebolt' ),
				'multiple'         => true,
			...
			'trim_title_chars' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Title chars trimmed count', 'tm-codebolt' ),
			),
			...
			'current_title_visibility' => array(
				...
				'label' => esc_html__( 'Display current navigation title', 'tm-codebolt' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'tm-codebolt' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'tm-codebolt' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'tm-codebolt' ),
			),
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'tm_codebolt_news_smart_box_default_query_args', array(
			'posts_per_page' => '5',
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'tm-codebolt' ) . '</h4></div>';
		}
...
add_action( 'widgets_init', 'tm_codebolt_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function tm_codebolt_register_news_smart_box_widgets() {
	register_widget( 'tm_codebolt_News_Smart_Box_Widget' );
}
