...
if ( ! class_exists( 'tm_codebolt_Playlist_Slider_Widget' ) ) {

	/**
	 * Class tm_codebolt_Playlist_Slider_Widget.
	 */
	class tm_codebolt_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'tm-codebolt' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'tm-codebolt' );
			$this->widget_id          = apply_filters( 'tm_codebolt_playlist_slider_widget_ID', 'tm_codebolt_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'tm_codebolt_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = tm_codebolt_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'tm-codebolt' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'tm-codebolt' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'tm-codebolt' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'tm-codebolt' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'tm-codebolt' ),
						true  => esc_html__( 'Buttons', 'tm-codebolt' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'tm-codebolt' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'tm-codebolt' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'tm-codebolt' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'tm-codebolt' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
				...
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'tm-codebolt' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'tm-codebolt' ),
						'author'   => esc_html__( 'Author', 'tm-codebolt' ),
						'comments' => esc_html__( 'Comment count', 'tm-codebolt' ),
						'category' => esc_html__( 'Category', 'tm-codebolt' ),
						'post_tag' => esc_html__( 'Tag', 'tm-codebolt' ),
					),
					'label' => esc_html__( 'Display post meta data', 'tm-codebolt' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'tm-codebolt' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'tm_codebolt_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
					'size'        => 'tm_codebolt-thumb-s',
					'mobile_size' => 'tm_codebolt-thumb-s',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'tm_codebolt_playlist_buttons', false ),
...
add_action( 'widgets_init', 'tm_codebolt_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function tm_codebolt_register_playlist_slider_widget() {
	register_widget( 'tm_codebolt_Playlist_Slider_Widget' );
}
