...
if ( ! class_exists( 'tm_codebolt_About_Widget' ) ) {

	/**
	 * Class tm_codebolt_About_Widget.
	 */
	class tm_codebolt_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'tm-codebolt' );
			$this->widget_id          = 'tm_codebolt_widget_about';
			$this->widget_name        = esc_html__( 'About tm_codebolt', 'tm-codebolt' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'tm-codebolt' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'tm-codebolt' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'tm-codebolt' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'tm-codebolt' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'tm-codebolt' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, tm_codebolt_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'tm_codebolt_widget_about_template_file', 'inc/widgets/about/views/about.php' ), false, false );
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'tm_codebolt_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'tm_codebolt_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'tm_codebolt_register_about_widget' );
/**
 * Register about widget.
 */
function tm_codebolt_register_about_widget() {
	register_widget( 'tm_codebolt_About_Widget' );
}
