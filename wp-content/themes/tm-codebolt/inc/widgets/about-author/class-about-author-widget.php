 ...
 * @package Tm_codebolt
 */
if ( ! class_exists( 'tm_codebolt_About_Author_Widget' ) ) {

	/**
	 * Class tm_codebolt_About_Author_Widget.
	 */
	class tm_codebolt_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'tm_codebolt widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'tm-codebolt' );
			$this->widget_id          = 'tm_codebolt_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'tm-codebolt' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'tm-codebolt' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'tm-codebolt' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'tm-codebolt' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'tm-codebolt' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'tm-codebolt' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'tm-codebolt' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'tm-codebolt' ),
					'label' => esc_html__( 'Link label', 'tm-codebolt' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'tm-codebolt' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'tm_codebolt-author-avatar' );
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'tm_codebolt_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'tm_codebolt_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function tm_codebolt_register_about_author_widgets() {
	register_widget( 'tm_codebolt_About_Author_Widget' );
}
